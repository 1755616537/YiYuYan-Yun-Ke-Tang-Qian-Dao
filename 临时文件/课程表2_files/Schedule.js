...
mui.ready(function () {
    sessionStorage.removeItem("play_status");
    //普通示例
        ...
        $.ready(function () {
            ...
            if (mydata) {
                ...
                wx.config({
                    ...
                    jsApiList: ['translateVoice', 'getLocation', 'scanQRCode'] // 必填，需要使用的JS接口列表，所有JS接口列表见附录2
                });
            }

            var stu_location_count = document.getElementById('stu_location_count').value;
            if (parseInt(stu_location_count) == 0) {
                wx.ready(function () {
                    wx.getLocation({
                        ...
                        success: function (res) {
                            ...
                            var data = {
                                ...
                                /*换成自己申请的key*/
                                key: "XTVBZ-ISVC6-QDYSL-MD54O-DQMQ7-XGBWP",
                                get_poi: 0
                            }
                            data.output = "jsonp";
                            jQuery.ajax({
                                ...
                                success: function (json) {
                                    ...
                                    if (json.status == 0) {
                                        mui.ajax('/OperatorLocation/InsertLocation', {
                                            data: {
                                                Latitude: res.latitude,
                                                Longitude: res.longitude,
                                                Nation_Code: json.result.ad_info.nation_code,
                                                Ad_Code: json.result.ad_info.adcode,
                                ...
                                error: function (err) {
                                    console.log("服务端错误，请刷新浏览器后重试。" + err);
                                }
                            });
                        }
                    });
